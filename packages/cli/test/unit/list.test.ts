...
describe('List Command', () => {
  ...
  it('renders table of runs with default limit', async () => {
    ...
    const { lastFrame, waitUntilExit } = render(<List options={{ limit: 20 }} />);
    await waitUntilExit();
  ...
  it('renders JSON output when --json flag is provided', async () => {
    ...
    const { lastFrame, waitUntilExit } = render(<List options={{ limit: 20, json: true }} />);
    await waitUntilExit();
  ...
  it('handles custom limit option', async () => {
    ...
    const { waitUntilExit } = render(<List options={{ limit: 1 }} />);
    await waitUntilExit();
