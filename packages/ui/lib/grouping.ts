 ...
 */
export const categorizeValue = (values: number[], value: number): GroupCategory => {
 ...
 */
export const calculateSuccessRate = (executions: Array<{ status: string }>): number => {
 ...
 */
export const calculateAverageTime = (executions: Array<{ time: number }>): number => {
  if (executions.length === 0) return 0;
